...
async function postBuild(clientOutDir, basePathname2, userStaticPaths, format, cleanStatic) {
  const ingorePathnames = /* @__PURE__ */ new Set([basePathname2 + "build/", basePathname2 + "assets/"]);
  ...
  if (import_node_fs.default.existsSync(clientOutDir)) {
    await loadDir(clientOutDir, basePathname2);
  }
  const notFoundPathsCode = createNotFoundPathsModule(basePathname2, notFounds, format);
  const staticPathsCode = createStaticPathsModule(basePathname2, staticPaths, format);
  return {
    notFoundPathsCode,
    staticPathsCode
  };
}
function createNotFoundPathsModule(basePathname2, notFounds, format) {
  notFounds.sort((a, b) => {
...
function createStaticPathsModule(basePathname2, staticPaths, format) {
    ...
    async generate({ serverOutDir }) {
      if (opts.functionRoutes !== false) {
        ...
        await import_node_fs3.default.promises.writeFile(
          (0, import_node_path3.join)(netlifyEdgeFnsDir, "manifest.json"),
