...
// packages/qwik-city/src/middleware/request-handler/server-error.ts
...
// packages/qwik-city/src/middleware/request-handler/rewrite-handler.ts
...
var loadRoute = async (routes, menus, cacheModules, pathname, isInternal) => {
...
import {
  AbortMessage as AbortMessage3,
  RedirectMessage as RedirectMessage3,
  ServerError as ServerError3,
  RewriteMessage as RewriteMessage3
...
async function runNext(requestEv, rebuildRouteInfo, resolve) {
  try {
    ...
    return new ServerError2(status, message);
  }
  ...
  async function _runNext() {
    try {
      ...
      if (e instanceof RedirectMessage2) {
        const stream = requestEv.getWritableStream();
        await stream.close();
      } else if (e instanceof RewriteMessage2) {
      ...
      } else if (e instanceof ServerError2) {
      ...
      } else if (!(e instanceof AbortMessage2)) {
...
function createRequestEvent(serverRequestEv, loadedRoute, requestHandlers, trailingSlash, basePathname, qwikSerializer, resolved) {
  ...
  const requestEv = {
    ...
    error: (statusCode, message) => {
      ...
      return new ServerError3(statusCode, message);
    },
    ...
    rewrite: (pathname2) => {
      ...
      return new RewriteMessage3(pathname2.replace(/\/+/g, "/"));
    },
    defer: (returnData) => {
...
var getArrayPaths = (formData) => {
  ...
  return new Set(
    Array.from(arrayCandidates.entries()).filter(([, isArrayPath]) => isArrayPath).map(([path]) => path)
  );
};
...
import { RedirectMessage as RedirectMessage4, ServerError as ServerError4 } from "@builder.io/qwik-city/middleware/request-handler";
    ...
    if (Array.isArray(data)) {
      ...
      if (isQrl(qrl) && qrl.getHash() === fn) {
        ...
        try {
          ...
          if (err instanceof ServerError4) {
...
async function handleQDataRedirect(requestEv) {
  try {
    ...
    if (!(err instanceof RedirectMessage4)) {
      throw err;
    }
  }
  if (requestEv.headersSent) {
...
async function requestHandler(serverRequestEv, opts, qwikSerializer) {
  ...
  if (routeAndHandlers) {
    ...
    const rebuildRouteInfo = async (url) => {
      const { pathname: pathname2 } = getRouteMatchPathname(url.pathname, qwikCityPlan.trailingSlash);
...
async function loadRequestHandlers(qwikCityPlan, pathname, method, checkOrigin, renderFn, isInternal) {
