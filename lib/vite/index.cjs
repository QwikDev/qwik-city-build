...
var require_merge = __commonJS({
  "node_modules/.pnpm/yaml@2.6.1/node_modules/yaml/dist/schema/yaml-1.1/merge.js"(exports2) {
    ...
    function addMergeToJSMap(ctx, map3, value2) {
      ...
      if (identity.isSeq(value2))
        ...
        mergeValue(ctx, map3, value2);
    }
    function mergeValue(ctx, map3, value2) {
  ...
  "node_modules/.pnpm/yaml@2.6.1/node_modules/yaml/dist/nodes/addPairToJSMap.js"(exports2) {
    ...
    function addPairToJSMap(ctx, map3, { key, value: value2 }) {
      if (identity.isNode(key) && key.addToJSMap)
        key.addToJSMap(ctx, map3, value2);
      else if (merge2.isMergeKey(ctx, key))
        merge2.addMergeToJSMap(ctx, map3, value2);
      else {
...
// packages/qwik-city/src/buildtime/vite/index.ts
var index_exports = {};
...
var qwik_city_sw_register_build_default = '"serviceWorker"in navigator?(navigator.serviceWorker.register("__url").catch(e=>console.error(e)),"caches"in window&&caches.keys().then(e=>{const r=e.find(c=>c.startsWith("QwikBuild"));r&&caches.delete(r).catch(console.error)}).catch(console.error)):console.log("Service worker not supported in this browser.")';
...
function createNotFoundPathsModule(basePathname, notFounds, format) {
  ...
  if (!notFounds.some((r2) => r2[0] === basePathname)) {
    const html5 = (0, import_request_handler.getErrorHtml)(404, "Resource Not Found");
...
var import_request_handler3 = require("@builder.io/qwik-city/middleware/request-handler");
...
var import_request_handler2 = require("@builder.io/qwik-city/middleware/request-handler");
var asyncStore;
...
async function runNext(requestEv, rebuildRouteInfo, resolve4) {
  try {
    ...
    const html5 = getErrorHtml2(status, message);
    requestEv.html(status, html5);
    return new import_request_handler2.ServerError(status, message);
  }
  ...
  async function _runNext() {
    try {
      ...
      if (e instanceof import_request_handler2.RedirectMessage) {
        const stream = requestEv.getWritableStream();
        await stream.close();
      } else if (e instanceof import_request_handler2.RewriteMessage) {
      ...
      } else if (e instanceof import_request_handler2.ServerError) {
      ...
      } else if (!(e instanceof import_request_handler2.AbortMessage)) {
...
function createRequestEvent(serverRequestEv, loadedRoute, requestHandlers, trailingSlash, basePathname, qwikSerializer, resolved) {
  ...
  const exit2 = () => {
    ...
    return new import_request_handler3.AbortMessage();
  };
  const loaders = {};
  const requestEv = {
    ...
    error: (statusCode, message) => {
      ...
      return new import_request_handler3.ServerError(statusCode, message);
    },
    redirect: (statusCode, url2) => {
      ...
      routeModuleIndex = ABORT_INDEX;
      return new import_request_handler3.RedirectMessage();
    },
    rewrite: (pathname2) => {
      check();
      ...
      return new import_request_handler3.RewriteMessage(pathname2.replace(/\/+/g, "/"));
    },
    defer: (returnData) => {
...
var isDangerousKey = (k) => k === "__proto__" || k === "constructor" || k === "prototype";
var isArrayIndexKey = (k) => /^(0|[1-9]\d*)$/.test(k);
var getArrayPaths = (formData) => {
  ...
  for (const [name] of formData) {
    ...
    for (let i = 0; i < keys2.length - 1; i++) {
      ...
      path3 = path3 ? `${path3}.${key}` : key;
      if (!arrayCandidates.has(path3)) {
        arrayCandidates.set(path3, true);
      }
      if (!isArrayIndexKey(keys2[i + 1])) {
        arrayCandidates.set(path3, false);
      }
    }
  }
  return new Set(
    Array.from(arrayCandidates.entries()).filter(([, isArrayPath]) => isArrayPath).map(([path3]) => path3)
  );
};
  ...
  for (const [name, value2] of formData) {
    ...
    for (let i = 0; i < keys2.length; i++) {
      ...
      if (i < keys2.length - 1) {
        ...
        object = object[key] = arrayPaths.has(path3) ? [] : /* @__PURE__ */ Object.create(null);
      } else {
        object[key] = value2;
      }
    }
...
var import_request_handler4 = require("@builder.io/qwik-city/middleware/request-handler");
var resolveRequestHandlers = (serverPlugins, route, method, checkOrigin, renderHandler, isInternal) => {
  const routeLoaders = [];
  const routeActions = [];
    ...
    if (Array.isArray(data)) {
      ...
      if (isQrl(qrl) && qrl.getHash() === fn) {
        ...
        try {
          ...
          if (err instanceof import_request_handler4.ServerError) {
...
function checkCSRF(requestEv, laxProto) {
  ...
  if (isSimpleRequest) {
    ...
    if (forbidden && laxProto && (inputOrigin == null ? void 0 : inputOrigin.replace(/^http(s)?/g, "")) === origin.replace(/^http(s)?/g, "")) {
      forbidden = false;
    }
    if (forbidden) {
...
async function handleQDataRedirect(requestEv) {
  try {
    ...
    if (!(err instanceof import_request_handler4.RedirectMessage)) {
      throw err;
    }
  }
  if (requestEv.headersSent) {
...
function getContentType(headers) {
  var _a;
  return (((_a = headers.get("content-type")) == null ? void 0 : _a.split(/[;,]/, 1)[0].trim()) ?? "").toLowerCase();
}
...
function ssrDevMiddleware(ctx, server) {
  ...
  return async (req, res, next) => {
    try {
      ...
      try {
        const { serverPlugins, loadedRoute } = await resolveRoute2(routeModulePaths, pathname);
        ...
        if (requestHandlers.length > 0) {
          ...
          const rebuildRouteInfo = async (url2) => {
            const { pathname: pathname2 } = getRouteMatchPathname(url2.pathname, ctx.opts.trailingSlash);
            const { serverPlugins: serverPlugins2, loadedRoute: loadedRoute2 } = await resolveRoute2(routeModulePaths, pathname2);
