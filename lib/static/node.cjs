...
async function createNodeMainProcess(sys, opts) {
  ...
  if (sitemapOutFile) {
    ...
    sitemapStream = import_node_fs.default.createWriteStream(sitemapOutFile, {
      flags: "w"
    });
    sitemapStream.write(
...
async function createNodeWorkerProcess(onMessage) {
  ...
  (_a = import_node_worker_threads2.parentPort) == null ? void 0 : _a.on("message", async (msg) => {
    ...
    if (msg.type === "close") {
      (_b = import_node_worker_threads2.parentPort) == null ? void 0 : _b.close();
    }
  });
}
...
async function workerRender(sys, opts, staticRoute, pendingPromises, callback) {
  ...
  try {
    ...
    const promise = (0, import_request_handler2.requestHandler)(requestCtx, opts, qwikSerializer).then((rsp) => {
      if (rsp != null) {
