...
const QwikCity = /* @__PURE__ */ qwik.componentQrl(qwik.inlinedQrl(() => {
  const env = useQwikCityEnv();
  if (!env?.params)
  ...
  qwik.useWatchQrl(qwik.inlinedQrl(async ({ track }) => {
    ...
    const { routes, menus, cacheModules } = await Promise.resolve().then(() => /* @__PURE__ */ _interopNamespace(require("@qwik-city-plan")));
    ...
    const endpointResponse = build.isServer ? env2.response : loadClientData(url2.href);
...
const Link = /* @__PURE__ */ qwik.componentQrl(qwik.inlinedQrl((props) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("a", {
    ...
    onMouseOver$: qwik.inlinedQrl((_, elm) => prefetchLinkResources(elm), "Link_component_a_onMouseOver_skxgNVWVOT8"),
    onQVisible$: qwik.inlinedQrl((_, elm) => prefetchLinkResources(elm, true), "Link_component_a_onQVisible_uVE5iM9H73c"),
